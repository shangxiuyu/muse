...
### 🚫 严格禁止场景 (Anti-Patterns / When NOT to use)
* **温馨母婴、儿童玩具与家庭生活日用**：冷峻的深蓝和暗夜极客氛围会破坏亲子产品的温馨感。
* **生鲜超市与餐饮外卖促销前台**：暗黑科技感会严重抑制食欲与日常冲动下单。
* **慢节奏文学阅读、日记手账与心灵冥想**：更适合暖米纸本文具流（如 Playful Stationery / Writer's Atelier）。

---

## 2. 母体之魂与核心基因 (Soul & Core Ethos)

> 💡 **母体之魂**：**极客对尖端物理秩序与深邃未知的敬畏——营造如太空飞船与超算指挥中心般的严密先锋与精密掌控感**。坚决摒弃虚浮假大空的“商业插画与套话”，通过午夜深海幽蓝（`#061220`）、电光赛车蓝（`#0075FF`）的视觉锚点、1px 晶体发光微边框与“用真实交互说话（Show, Don't Tell）”的操作舱，让用户在 5 秒内触摸到技术底层的硬核确定性。

* **午夜深海冷蓝底（`#061220` ~ `#0B1929`）与径向环境微光**：绝非死沉压抑的纯黑，具有无限深邃的太空纵深感。
* **高反差纯白按键（`#FFFFFF`）配纯黑字**：在深色大底上形成无可抗拒的视觉吸附力与点击推力。
* **1px 晶体发光微边框（Subtle Glow Border）**：半透明晶体包裹，营造悬浮于星舰操作台的精密工业质感。
* **“Show, Don't Tell” Live 拟真操作舱**：首屏直接演示活体可交互代码、终端或产品微界面。

---

## 3. 三大排版拓扑发散矩阵 (Three Layout Paradigms & Mutations)

> ⚡ **AI 消费指引 (学魂变骨)**：科技旗舰绝不仅有单一官网首屏！根据产品定位选择或变异以下骨架：

```
┌─────────────────────────────────────────────────────────────────────────────────────────────────┐
│                          Tech Flagship Dark 三大拓扑发散矩阵                                    │
├────────────────────────────────┬────────────────────────────────┬──────────────────────────────┤
│ 范式 A：深海浮空操作舱 (Cockpit)│ 范式 B：发布长画卷 (Narrative) │ 范式 C：节点拓扑矩阵 (Matrix) │
├────────────────────────────────┼────────────────────────────────┼──────────────────────────────┤
│ 适用：AI 工具平台、SaaS 官网   │ 适用：智能硬件首发、旗舰新版本 │ 适用：多云运维、安全防御大屏 │
└────────────────────────────────┴────────────────────────────────┴──────────────────────────────┘
```

### 范式 A：【先锋硬核】深海浮空拟真操作舱 (Floating Cockpit & Live Telemetry)
* **设计意图**：左侧极度克制的大标题叙事，右侧直接悬浮可交互的 macOS 终端/产品舱，底部无缝羽化流动。
* **适用场景**：开发者工具平台、AI Agent 工作流首发、新一代专业级桌面软件。
* **拓扑骨架 (ASCII Layout)**：
  ```
  ┌─────────────────────────────────────────────────────────────────────────────┐
  │ [极简深蓝毛玻璃顶栏：LOGO + 全键盘 Command Menu 搜索 + 纯白反差 CTA]       │
  ├──────────────────────────────────────┬──────────────────────────────────────┤
  │ 左侧：旗舰叙事与行动点              │ 右侧：拟真 macOS 悬浮发光操作舱      │
  │ • 56px 工业大标题                    │ ┌──────────────────────────────────┐ │
  │   "Ship AI Agents with               │ │ ● ● ●  Live Telemetry v4.2       │ │
  │    <span style="color:#0075FF;">Zero Latency</span>."                │ ├──────────────────────────────────┤ │
  │ • 一句式工程解释                     │ │ $ curl -X POST https://api...    │ │
  │ • [高反差纯白按钮: GET API KEY]      │ │ ➜ 200 OK (14ms, 99.99% uptime)   │ │
  │ • 极细 JetBrains Mono 版本戳         │ └──────────────────────────────────┘ │
  ├──────────────────────────────────────┴──────────────────────────────────────┤
  │ [双端渐变羽化遮罩 · 恒速向左流动的客户/指标动脉 Marquee Telemetry Stream]    │
  └─────────────────────────────────────────────────────────────────────────────┘
  ```

### 范式 B：【巨幕美学】Apple/Vercel 式叙事发布长画卷 (Narrative Scroll Storyboard)
* **设计意图**：单栏宽幅居中巨幕，硬件级精密 3D 渲染图或芯片级剖面卡片居中，文字渐进滚动高亮。
* **适用场景**：高端硬件首发（如 Nothing 手机/耳机）、AI 硬件终端、年度大版本发布。
* **拓扑骨架 (ASCII Layout)**：
  ```
  ┌─────────────────────────────────────────────────────────────────────────────┐
  │                                                                             │
  │                     TITANIUM ARCHITECTURE. AT SCALE.                        │
  │             (Newsreader 64px 银白高亮 / 渐入微动效)                         │
  │                                                                             │
  │       ┌─────────────────────────────────────────────────────────────┐       │
  │       │                                                             │       │
  │       │       [ 1200px 巨幕硬件超精密微距微光剖面卡片 / 视频 ]      │       │
  │       │       1px 发光晶体边框 + 内部局部放大雷达刻度               │       │
  │       │                                                             │       │
  │       └─────────────────────────────────────────────────────────────┘       │
  │                                                                             │
  │   [ 3 组对称极致指标: 3.2GHz Core / 0.8ms Reaction / 100% Solid Aluminum ] │
  └─────────────────────────────────────────────────────────────────────────────┘
  ```

### 范式 C：【极客中枢】多节点拓扑与遥测矩阵 (Node Topology & Mesh Matrix)
* **设计意图**：深色微点阵网格（Dot Grid）背景，多张发光卡片以拓扑节点连线串联，实时更新遥测数据。
* **适用场景**：分布式系统监控、安全防御攻防演练大屏、量化交易节点大盘。
* **拓扑骨架**：由 1px 晶体微边框卡片咬合形成多维度矩阵，每张卡片右上方带绿色呼吸状态指示灯（Pulsing Beacon）。

---

## 4. 字体与排印体系 (Typography Tokens)
...
## 6. 专属排版手艺与组件代码 (Craft & Code Snippets)
...
## 7. 交互与动效参数 (Motion & Micro-interactions)
...
* 🚫 **严禁单一官网模板思维**：必须在浮空操作舱、巨幕叙事画卷与网格遥测矩阵间根据业务场景自如切换，严禁无脑复制代码！
