...
* **子项按钮**：
  ...
  * 过渡：`all 150ms cubic-bezier(0.16, 1, 0.3, 1)`。
...
* **最大行长限制**：正文编辑区域最大宽度严格锁定为 `780px` (对齐人眼舒适扫描范围 `65~75ch`)，居中留白；
...
* **正文排版规范**：
  ...
  * 引用块 `blockquote`：左侧保留 `3px solid var(--accent-amber)`，背景 `var(--bg-surface-subtle)`，右侧圆角 `10px`；
...
1. **Spring 弹簧物理曲线**：
   * 所有悬浮、折叠、位移统一采用：`cubic-bezier(0.16, 1, 0.3, 1)`（无振荡、平滑刹车微回弹）；
   * 极速响应时长：`150ms`；标准过渡时长：`240ms`。
...
| **🚫 无节制的通栏横拉正文** | **✅ 正文行长严格限制在 `780px` (`max-w-[75ch]`) 以内** |
